...
data_cal = np.genfromtxt(os.path.join(dirname, "data/data_26_07_2022/data.csv"), delimiter=',')
data_no_cal = np.genfromtxt(os.path.join(dirname, "data/data_26_07_2022/data_no_cal.csv"), delimiter=',')
...
fig.suptitle(f"Angles of Maximum power ({len(angles_cal)} trys)", fontsize=12)
...
fig.suptitle(f"PLLs phase differences ({len(angles_cal)} trys)", fontsize=12)
